...
def test_wait_for_api_key_reads_server_config(tmp_path: Path) -> None:
    ...
    (jackett_dir / "ServerConfig.json").write_text(
        '{"APIKey": "abc123"}', encoding="utf-8"
    )
...
def test_compose_up_builds_expected_command(
    ...
    (plugin_root / "compose" / "docker-compose.override.yml").write_text(
        "version: '3.9'\n", encoding="utf-8"
    )
    (plugin_root / "phelia.yaml").write_text(
        "schema: 1\nid: test\nname: test\nversion: 0.0.1\nphelia:\n  minVersion: '0.7.0'\n  hooks:\n    backend:\n      entrypoint: dummy\n",
        encoding="utf-8",
    )
