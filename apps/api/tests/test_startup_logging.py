...
def test_init_db_exception_logged(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        main, "load_provider_credentials", lambda db: None, raising=False
    )
    ...
    assert any(
        "Error initializing database" in record.message for record in caplog.records
    )
...
def test_provider_credential_exception_logged(monkeypatch, caplog):
    ...
    assert any(
        "Error loading provider credentials" in record.message
...
def test_qb_health_check_exception_logged(monkeypatch, caplog):
    ...
    assert any(
        "Error checking qBittorrent connectivity" in record.message
        for record in caplog.records
    )
